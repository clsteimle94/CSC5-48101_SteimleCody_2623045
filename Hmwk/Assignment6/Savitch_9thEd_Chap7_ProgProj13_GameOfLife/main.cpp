/* 
   ...
   Purpose:  Game of Life using a 3 Dimensional Array
 */
...
void glider(char [][ROWS][COLS],int,int,int);        //Glider objects
void random(char [][ROWS][COLS],int);                //Random objects
void neybor(char [][ROWS][COLS],int,int [][COLS]);   //Determines neighbors
void nextGen(char [][ROWS][COLS],int,int [][COLS]);  //The next generation
void pause(int);                                     //Pauses the display
...
int main(int argc, char** argv) {
    ...
    //Initialize
    cout<<"This program will go through 10 generations of the Game of Life."<<endl;
...
void nextGen(char life[][ROWS][COLS],int nGEN,int count[][COLS]){
    ...
    for(int row=0;row<ROWS;row++){
        for(int col=0;col<COLS;col++){
            ...
            if(count[row][col]==3)life[nGEN+1][row][col]='*'; //Birth
        ...
        for(int col=0;col<COLS;col++){
            for(int roff=row-1;roff<=row+1;roff++){
                for(int coff=col-1;coff<=col+1;coff++){
                    ...
                    if(life[nGen][roff][coff]=='*')count[row][col]++;
                }
            }
        }
    }
...
void glider(char life[][ROWS][COLS],int gen,int row,int col){
    life[gen][row+0][col+1]='*';
    life[gen][row+1][col+2]='*';
    life[gen][row+2][col+0]='*';
    life[gen][row+2][col+1]='*';
    life[gen][row+2][col+2]='*';
}
